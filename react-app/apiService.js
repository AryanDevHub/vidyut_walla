...
/**
 * API Service - Mocks a backend service for fetching and updating data.
 */
class ApiService {
  getCurrentStatus() {
    return fakeFetch(MOCK_DATA.current_status);
  }

  getHistoricalData() {
    return fakeFetch(MOCK_DATA.historical_data);
  }

  getForecasts() {
    return fakeFetch(MOCK_DATA.forecasts);
  }

  getRecommendations() {
    return fakeFetch(MOCK_DATA.recommendations);
  }

  getAlerts() {
    return fakeFetch(MOCK_DATA.alerts);
  }
  ...
  updateLoadSchedule(schedule) {
    console.log('API: Updating load schedule', schedule);
    return fakeFetch({ success: true, schedule });
  }
}
